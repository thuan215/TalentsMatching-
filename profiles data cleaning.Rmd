...
```{r 3}
# profile taxonomy
analyst<-c("data analyst", "data analist", "data specialist", "data", "business intelligence")

it<-c("business analist","project manager","business analyst",
      "product_owner","ux designer", "user experience")
...
profiles_dev$target_var<-rep(1,nrow(profiles_dev)) #assign label=1 to dev profiles

## Extract other profiles
profiles_it<-extract_profiles(talent_pool = talent_bigset,roles = it)
profiles_analyst<-extract_profiles(talent_pool = talent_bigset,roles = analyst)
profiles_nonit<-extract_profiles(talent_pool = talent_bigset,roles = nonit)

profiles_others<-bind_rows(profiles_it, profiles_analyst, profiles_nonit)
  ### Filter for the skills that appear in at least x% of all the profiles
profiles_others<-profiles_others[,colSums(profiles_others[,-1])>0.05*nrow(profiles_others),with=FALSE]
profiles_others$target_var<-rep(0,nrow(profiles_others)) #assign label=0 to other profiles

## Filter for the skills that appear in at least x% of all the profiles
profiles<-profiles[,colSums(profiles[,-1])>0.05*nrow(profiles),with=FALSE]
...
## Save to csv
```{r 5}
write.csv(profiles_final,"profiles_final.csv", row.names = FALSE)
```
